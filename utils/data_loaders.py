...
class InferenceDataset(torch.utils.data.dataset.Dataset):
    """Inference Data class used for PyTorch DataLoader"""
    ...
    def __getitem__(self, idx):
        taxonomy_name, sample_name, rendering_images = self.get_datum(idx)
    ...
    def get_datum(self, idx):
        ...
        return taxonomy_name, sample_name, np.asarray(rendering_images)


class InferenceDataLoader:
    def __init__(self, cfg):
        self.dataset_taxonomy = None
        self.rendering_image_path_template = cfg.DATASETS.INFERENCE.RENDERING_PATH

        # Load all taxonomies of the dataset
        with open(cfg.DATASETS.INFERENCE.TAXONOMY_FILE_PATH, encoding='utf-8') as file:
            self.dataset_taxonomy = json.loads(file.read())
    ...
    def get_dataset(self, dataset_type, n_views_rendering, transforms=None):
        ...
        print('[INFO] %s Complete collecting files of the dataset. Total files: %d.' % (dt.now(), len(files)))
        return InferenceDataset(dataset_type, files, n_views_rendering, transforms)
